...
export async function POST(req: NextRequest) {
  ...
  try {
    updateSession(sessionId, { stage: "extracting_questions" });
    const questions = await extractQuestions(session.questionPaperImages);
    ...
    const updated = updateSession(sessionId, { questions })!;
    return NextResponse.json({ session: toPublic(updated) });
  } catch (err) {
    console.error("extract-questions error", err);
    updateSession(sessionId, { stage: "error", error: "Failed to extract questions from the question paper." });
