...
export async function POST(req: NextRequest) {
  ...
  try {
    updateSession(sessionId, { stage: "grading" });
    const { grades, overall } = await gradeAnswers(session.questions, session.answers);
    const updated = updateSession(sessionId, { grades, overall, stage: "done" })!;
    return NextResponse.json({ session: toPublic(updated) });
  } catch (err) {
    console.error("grade error", err);
    updateSession(sessionId, { stage: "error", error: "Failed to grade answers." });
